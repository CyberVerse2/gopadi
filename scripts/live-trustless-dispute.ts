...
async function main() {
  ...
  console.log("7. Opening dispute...");
  const dispute = await trustlessPost(
  ...
  await submitTrustlessXdr(assertUnsigned(dispute, "dispute"), platform, apiKey);

  console.log("8. Resolving dispute with refund to customer...");
  const resolve = await trustlessPost(
    "/escrow/single-release/resolve-dispute",
    {
      contractId,
      disputeResolver: resolver.publicKey(),
      distributions: [{ address: platform.publicKey(), amount: 1 }],
    },
    apiKey,
  );
  const resolveSubmit = await submitTrustlessXdr(assertUnsigned(resolve, "resolve dispute"), resolver, apiKey);
    ...
    JSON.stringify(
      {
        ...
        resolveStatus: resolveSubmit.status,
      },
      null,
      2,
    ),
